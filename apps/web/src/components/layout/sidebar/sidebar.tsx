...
export function Sidebar() {
  ...
  const { collections, openCollections, toggleCollection } =
  ...
  return (
    <>
      <aside className="flex w-63.75 h-screen shrink-0 flex-col border-r-[1.25px] border-[#E5E5E5]">
        ...
        <Link
          to="/"
          className="pl-3 w-full h-16 flex items-center gap-2 border-b-[1.25px] border-[#E5E5E5]"
        >
                ...
                {collection.requests.map((request) => (
                  <SidebarSubItem
                    ...
                    to={buildWorkspacePath(
                      currentWorkspaceId,
                      `/collections/${collection.id}/requests/${request.id}`,
                    )}
